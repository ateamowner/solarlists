...
- Homepage JSON-LD only: `LocalBusiness` (`https://solarlists.com/#organization`, A Team Contracting, in-house solar quotes — not a marketplace Organization), `WebSite` with `publisher.@id` pointing at that LocalBusiness, and `FAQPage` matching the five visible homepage FAQs. No SearchAction. No invented phone, street, reviews, or ratings in schema.
